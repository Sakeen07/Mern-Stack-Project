    ...
    <div>
        <form onSubmit={handleSubmit}>
            <Box width={'50%'} padding={10} margin="auto" display={"flex"} flexDirection="column" boxShadow={'10px 10px 20px #ccc'}>
                ...
                <Box display={"flex"}>
                    <TextField onChange={(e) => setActor(e.target.value)} value={actor} name='actor' variant='standard' margin='normal'/>
                    <Button onClick={()=> {
                ...
                <Checkbox name='featured' checked={inputs.featured} onChange={(e)=>setInputs((prevState)=>({...prevState, 
                featured: e.target.checked}))} sx={{mr:"auto"}}/>
